...
这是一个独立的命令行 Text2SQL 小项目：使用 LangGraph 编排问题澄清、Schema Linking、SQL 生成、执行错误修复与低置信度人工审核。Catalog、Chroma 和只读 SQLite 执行器仍由原模块提供。它有自己的 Git 仓库，不依赖外层 `openchatbi` 项目的 Python 包或运行配置。
...
用户输入问题
  ...
  → LangGraph 提取问题信息；必要时暂停询问用户，恢复后重新提取
...
入口是 [`main.py`](main.py)，图的节点和条件边定义在 [`workflow.py`](workflow.py)。模型和检索资源在图外创建，图状态只保存可持久化的查询数据。澄清与审核使用 SQLite 检查点及任务 ID 暂停、恢复。
...
[`schema_linking.py`](schema_linking.py) 的图专用提取函数第一次调用聊天模型，先判断是否必须澄清；明确的问题整理成 `QuestionInfo`：
...
上面仅是格式示例，具体内容由模型返回。需要澄清时，图用 `interrupt` 暂停；用户补充信息后用相同任务 ID 恢复，再次提取。最多澄清两轮。`rewrite_question` 是完整、忠实的改写；后三项都是 `list[str]`。当前 Mini 版把三组词合并用作字段检索线索，**没有**像外层原项目那样按维度和指标的字段类别分别过滤向量搜索。`keywords` 还会单独用于检索选表示例。
...
[`generate_sql.py`](generate_sql.py) 把改写问题和这段上下文交给同一个聊天模型，要求只返回一条 SQLite `SELECT` 查询。代码会移除可能出现的 Markdown 代码围栏；生成阶段仍不验证 SQL 的业务正确性。
...
图只对可修复的语法、字段、表、函数及聚合错误尝试重新生成 SQL，最多重试 3 次；超时和被拒绝的查询直接结束。成功执行后，[`sql_quality.py`](sql_quality.py) 请模型按问题、字段含义、SQL 和结果预览评分。低于 `0.7` 或评分失败时，图暂停等待人工批准、修改或拒绝。修改的 SQL 再走同一个只读执行节点，拒绝则在重试次数内重新生成。评分是审核线索，不保证业务答案正确。

[`presentation.py`](presentation.py) 将结果排成终端表格，显示失败、无数据或后续还有数据的提示。当前入口打印最终 SQL 和查询结果，**不生成自然语言说明**。
...
在提示符输入自然语言问题，例如“给我说出所有客户的名字和订单数量”。程序会打印任务 ID；若需要澄清或审核，会提示输入。此时按 Ctrl-C 或结束标准输入，稍后可使用 `python main.py --resume <任务 ID>` 从本地 `checkpoints.sqlite` 继续。程序完成后打印 SQL 和结果表格。示例数据库已随仓库提交，无需另行导入。

### 5. 可选：运行离线回归测试

```bash
python -m pip install pytest
python -m pytest -q -o addopts='' test_refactor.py test_workflow.py
```
